<script lang="ts">
  ...
  const navLinks: Array<{ id: string; label: string; icon: typeof HomeIcon; filter?: TimelineFilter }> = [
    ...
    onSubmit: async (payload) => {
      ...
      const status: Status = {
        ...
        mediaAttachments: (payload.mediaAttachments ?? []).map((media) => ({
          ...
          url: media.url ?? media.previewUrl ?? 'https://placehold.co/600x400/0f172a/ffffff?text=Media',
          previewUrl: media.previewUrl ?? media.url,
          description: media.description ?? media.file?.name ?? 'Attachment'
        })),
        mentions: [],
        tags: [{ name: timelineState.filter, url: `https://equalto.social/tags/${timelineState.filter}` }]
      };
      ...
      <section class="timeline-feed" aria-live="polite">
        <header>
          <div>
            ...
            <p class="muted">Prefetched {timelineState.prefetched} posts · Density {timelinePreferences.density}</p>
          </div>
          <div class="timeline-feed__actions">
            <Button size="sm" variant="outline" onclick={() => timeline.refresh()}>
              Refresh
            </Button>
        ...
        {#if timelineState.error}
          <div class="state-card state-card--error" role="alert">
            ...
            <Button size="sm" variant="ghost" onclick={() => timeline.clearError()}>
              Dismiss
            </Button>
          </div>
        {/if}
      ...
      <section class="pinned-card" aria-label="Pinned posts">
        ...
        <div class="pinned-card__list">
          {#each pinnedStatuses as status (status.id)}
            <StatusCard status={status} density="compact" />
          {/each}
        </div>
      </section>
              ...
              <div class="notification-row">
                ...
                <div>
                  <p>
                    ...
                    {#if notification.type === 'mention'} left a mention{/if}
                    {#if notification.type === 'follow'} followed you{/if}
                    {#if notification.type === 'favourite'} favorited your post{/if}
                    {#if notification.type === 'reblog'} boosted your post{/if}
                    {#if notification.type === 'follow_request'} requested to follow you{/if}
                  </p>
                  <span class="muted">{new Date(notification.createdAt).toLocaleTimeString()}</span>
                </div>
                {#if !notification.read}
                  <Button size="sm" variant="outline" onclick={() => markNotification(notification.id)}>
                    Mark read
                  </Button>
                {/if}
              </div>

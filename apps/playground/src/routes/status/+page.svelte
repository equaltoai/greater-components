<script lang="ts">
  ...
  import { StatusCard, Status, type StatusActionHandlers } from '@equaltoai/greater-components-fediverse';
  ...
  const threadRoot = showcaseStatuses.find((status) => status.id === 'status-thread-root') ?? showcaseStatuses[showcaseStatuses.length - 1]!;
...
<DemoPage eyebrow="Fediverse Surface" title={data.metadata.title} description={data.metadata.description}>
  <section class="status-section">
    <header>
      <p class="section-eyebrow">01 · Published StatusCard</p>
      <h2>Media, content warnings, and polls</h2>
      <p>
        All data comes from <code>createStatusShowcase()</code>, mirroring ActivityPub payloads with spoiler text,
        media attachments, and poll metadata. We tap into the published <code>StatusCard</code> component and wire
        the action callbacks so you can drop in API calls later.
      </p>
    </header>

    <div class="status-grid">
      <div class="status-card-stack" role="region" aria-live="polite">
        <StatusCard status={spoilerStatus} actionHandlers={actionHandlers} />
        <StatusCard status={pollStatus} actionHandlers={actionHandlers} density="comfortable" />
      </div>
  ...
  <section class="status-section">
    <header>
      ...
      <p>
        The root card reuses the "thread" entry from <code>createStatusShowcase()</code>, while replies are provided
        by <code>createThreadReplies()</code>. We blend <code>StatusCard</code> with <code>Status.*</code> to illustrate
        how to stitch a thread with consistent action handlers.
      </p>
    </header>
    ...
    <div class="thread-layout">
      <div class="thread-stack" role="region" aria-labelledby="thread-heading">
        ...
        <StatusCard status={threadRoot} actionHandlers={actionHandlers} />
  ...
  <section class="status-section">
    ...
    <div class="a11y-grid">
      ...
      <CodeExample title="Accessibility snippet" description="Role + shortcut annotations" code={a11ySnippet} />
    </div>
  </section>
</DemoPage>

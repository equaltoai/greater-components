<script lang="ts">
  ...
  import { Button, Tabs, TextField, TextArea, Modal, Avatar, Skeleton } from '@equaltoai/greater-components-primitives';
...
<DemoPage eyebrow="Fediverse Surface" title={data.metadata.title} description={data.metadata.description}>
  ...
  <section class="profile-section">
    ...
    <div class="pinned-rail">
      {#each pinned as status (status.id)}
        <StatusCard status={status} density="comfortable" />
      {/each}
    </div>
  </section>
  ...
  <section class="profile-section">
    <header class="profile-section__header">
      <div>
        ...
        <p class="muted">The tabs component switches between feed slices without remounting cards.</p>
      </div>
    </header>
  ...
  <section class="profile-section profile-section--grid">
    ...
    <p class="muted connections-note" aria-live="polite">{followToast || 'Follow toggles update counts optimistically.'}</p>
  </section>
</DemoPage>
...
{#snippet ProfileActionSnippet()}
  <Button size="sm" variant="solid" onclick={openEditModal}>
    Edit profile
  </Button>
{/snippet}

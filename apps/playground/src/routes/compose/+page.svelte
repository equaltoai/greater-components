<script lang="ts">
  ...
  async function simulateSubmit(
    data: { content: string; visibility: PostVisibility; contentWarning?: string; mediaAttachments?: ComposeAttachment[] },
    label: string
  ) {
    await wait(420);
    submissions = [
  ...
  const mediaHandlers: ComposeHandlers = {
    ...
    onMediaUpload: async (file) => {
      await wait(650);
      recentUploads = [
        { id: makeId(), name: file.name, bytes: file.size },
        ...recentUploads
      ].slice(0, 4);
  ...
  async function searchDirectory(
    ...
    if (type === 'mention') {
      return mentionDirectory
        .filter((entry) => entry.username.includes(normalized) || entry.displayName.toLowerCase().includes(normalized))
        .map((entry) => ({
          type: 'mention',
...
>
  ...
  <section class="compose-demo__section">
    <header>
      ...
      <p>
        ...
        drop. Content warnings live outside the composer in this demo, mirroring how many Fediverse
        clients expose a toggle.
      </p>
    </header>
  ...
  <section class="compose-demo__section">
    <header>
      ...
      <p>
        Mention autocomplete rides on <code>Compose.EditorWithAutocomplete</code>, while polls reuse the
        `PollComposer` pattern export so you can generate ActivityPub-compatible payloads before hitting
        your API.
      </p>
    </header>
      ...
      <div class="compose-demo__panel">
        ...
        {#if pollDraft}
          <div class="poll-summary" role="status">
            ...
            <Button variant="ghost" size="sm" onclick={() => (pollDraft = null)}>
              Clear poll
            </Button>
          </div>
        {/if}

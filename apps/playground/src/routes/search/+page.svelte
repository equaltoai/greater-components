...
<Search.Root handlers={handlers} initialQuery="compose dock">
  ...
  <DemoPage eyebrow="Search" title={data.metadata.title} description={data.metadata.description}>
    <section class="search-section">
      <header>
        ...
        <p class="muted">Semantic mode flips the Lesser API flag; filter chips scope results without refetching clients.</p>
      </header>
      <div class="search-surface">
        <Search.Bar placeholder="Search posts, people, tags…" autofocus />
        <Button size="sm" variant="outline" onclick={() => searchContext?.search()} disabled={searchLoading}>
      ...
      <aside class="side-column">
        <section class="side-card">
          <header>
            ...
            <Button size="sm" variant="ghost" onclick={clearRecents} disabled={recentSearches.length === 0}>
              Clear recents
            </Button>
          </header>

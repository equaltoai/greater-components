<script lang="ts">
	...
	function updateSettings<K extends keyof SettingsState>(
		section: K,
	...
	function togglePrivacy(field: keyof SettingsState['privacy']) {
		updateSettings('privacy', { [field]: !settings.privacy[field] } as Partial<
			SettingsState['privacy']
		>);
	}

	function toggleNotification(field: keyof SettingsState['notifications']) {
		updateSettings('notifications', { [field]: !settings.notifications[field] } as Partial<
		...
		<div class="settings-grid">
			<section class="settings-card">
				<header>
					...
					<h2>Theme & density</h2>
					<p class="muted">
						ThemeSwitcher hooks into the shared preferences store so previews stay on-brand.
					</p>
				</header>
				<ThemeSwitcher
					variant="full"
				...
				<div class="appearance-actions">
					<Button variant="ghost" size="sm" onclick={resetAppearance}>Reset appearance</Button>
				</div>
			</section>
			...
			<section class="settings-card">
				<header>
					...
					<p class="muted">
						Switches update instantly; we store the state locally so QA can refresh.
					</p>
				</header>
			...
			<section class="settings-card">
				<header>
					...
					<p class="muted">
						Digest cadence and channel toggles simulate optimistic preference updates.
					</p>
				</header>
				<div class="toggle-stack">
			...
			<section class="settings-card">
				...
				<TextField
					...
					oninput={(event) =>
						handleAccountChange('displayName', (event.currentTarget as HTMLInputElement).value)}
					required
				/>
				<TextField
					label="Location"
					...
					oninput={(event) =>
						handleAccountChange('location', (event.currentTarget as HTMLInputElement).value)}
				/>
				<TextArea
					label="Bio"
					rows={4}
					...
					oninput={(event) =>
						handleAccountChange('bio', (event.currentTarget as HTMLTextAreaElement).value)}
				/>
			</section>

<script lang="ts">
  ...
  const isDisplayNameInvalid = $derived(() => displayName.trim().length > 0 && displayName.trim().length < 2);
...
>
  <section class="demo-section">
    ...
    <p class="a11y-tip">Accessible pattern: help text is tied via aria-describedby and errors announce with role="alert" when invalid.</p>
  ...
  <section class="demo-section">
    ...
    <p class="a11y-tip">Tip: keep aria-live polite for counters so screen readers announce only when values change.</p>
  ...
  <section class="demo-section">
    ...
    <p class="a11y-tip">Keyboard: Up/Down cycles radios while typing the first letter jumps via native typeahead. Disabled select options remain unreachable.</p>
  ...
  <section class="demo-section">
    ...
    <p class="a11y-tip">Space toggles the focused checkbox or switch; disabled switches remove themselves from the tab order.</p>
  ...
  <section class="demo-section">
    <header>
      ...
      <p>Wrap the published FileUpload control with a drag surface for richer UX while keeping drag events in user land.</p>
    </header>

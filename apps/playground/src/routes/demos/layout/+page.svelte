...
>
	<section class="demo-section">
		<header>
			...
			<p>
				Mix photos, initials fallback, and live status indicators. The status pill reuses the same
				tokenized colors across variants.
			</p>
		</header>
		...
		<div class="avatar-controls">
			...
			<p class="status-callout">
				Status indicator remains visible to assistive tech via sr-only text baked into the
				component.
			</p>
		</div>

		<p class="a11y-tip">
			Tip: Tab to each avatar to confirm focus rings appear even when the component renders an
			image.
		</p>
	...
	<section class="demo-section">
		<header>
			...
			<p>
				Rectangular, text, and circular placeholders keep layout shifts minimal while content loads.
			</p>
		</header>
		...
		<p class="a11y-tip">
			Skeletons default to <code>role="status"</code>, so screen readers hear a loading announcement
			once.
		</p>
	...
	<section class="demo-section">
		<header>
			<h2>Modal Flow</h2>
			<p>
				Focus trapping, ESC close, and footer actions ship out of the box. Use aria-live text nearby
				to report the last choice.
			</p>
		</header>
		...
		<Modal bind:open={modalOpen} title="Review publication" closeOnBackdrop preventScroll>
			<p>
				This dialog demonstrates nested footer actions, ESC dismissal, and automatic focus sent back
			...
			{#snippet footer()}
				<div class="modal-footer">
					...
					<Button onclick={() => recordModalAction('Published from modal')}>Publish update</Button>
				</div>
			{/snippet}
		</Modal>

		<p class="a11y-tip">
			Keyboard: focus starts on the dialog. Press Shift+Tab to verify focus loop, or ESC to close.
		</p>
	...
	<section class="demo-section">
		<header>
			...
			<p>
				Underline tabs for horizontal navigation and manual-activation vertical tabs for settings
				panes.
			</p>
		</header>
		...
		<p class="a11y-tip">
			Arrow keys follow the orientation automatically. Manual activation waits for Enter/Space
	...
	<section class="demo-section">
		<header>
			...
			<p>
				Hover vs click interactions show how helpers expose placement props and custom triggers.
			</p>
		</header>
		...
		<p class="a11y-tip">
			Click-triggered tooltips stay open until focus or ESC closes them—remind users with inline
			copy.
		</p>

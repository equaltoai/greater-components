<script lang="ts">
  ...
  const aliasLookup = Object.entries(iconAliases).reduce<Record<string, string[]>>((acc, [alias, canonical]) => {
  ...
  const filteredIcons = $derived(() => {
    ...
    const pool = selectedCategory === 'all' ? iconList : iconCategories[selectedCategory as CategoryKey] ?? [];
    if (!query) return pool;
    return pool.filter((name) => name.includes(query) || aliasLookup[name]?.some((alias) => alias.includes(query)));
  });

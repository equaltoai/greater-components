<script lang="ts">
  ...
  type VariantConfig = { id: string; label: string; variant: (typeof variants)[number]; size: (typeof sizes)[number] };
...
<div class="button-grid">
  ...
  const iconButtons: IconButtonConfig[] = [
    { id: 'primary-prefix', label: 'Share update', variant: 'solid', affix: 'prefix', Icon: SendIcon },
    { id: 'secondary-suffix', label: 'Continue', variant: 'outline', affix: 'suffix', Icon: ArrowRightIcon },
...
<div class="icon-demo">
  ...
  function handleSubmit(event: Event) {
    ...
    submissionLog = [`Submitted at ${new Date().toLocaleTimeString()}`, ...submissionLog].slice(0, 2);
  }
...
>
  <section class="demo-section">
    <header>
      ...
      <p>Solid, outline, and ghost buttons rendered directly from the published primitives package in every supported size.</p>
    </header>
    ...
    <p class="a11y-tip">Tip: Each button remains reachable via Tab order regardless of variant—confirm focus outlines meet contrast guidance.</p>
  ...
  <section class="demo-section">
    <header>
      ...
      <p>Prefix/suffix snippets keep icon spacing aligned with the tokens package, and icon-only buttons carry aria-labels for assistive tech.</p>
    </header>
    ...
    <p class="a11y-tip">Tip: Provide aria-labels for icon-only buttons and ensure hover/click targets are at least 44×44px.</p>
  ...
  <section class="demo-section">
    <header>
      ...
      <p>Demonstrates button types, disabled/loading states, and a lightweight async simulation for user feedback.</p>
    </header>
    ...
    <div class="state-demo">
      ...
      <div>
        ...
        <form class="button-form" onsubmit={handleSubmit} onreset={handleReset}>
          ...
          <Button type="button" variant="ghost" onclick={() => (clickCount += 1)}>Secondary action</Button>
        </form>
    ...
    <p class="a11y-tip">Tip: Loading buttons automatically set `aria-busy` and remove them from tab order; communicate progress textually as well.</p>

...
`;
  ...
  const fullText = "Here is some streaming text that appears like magic! It simulates an AI response generating content token by token.";
...
<Container maxWidth="lg" padding="md">
  ...
  <Section>
    ...
    <Card padding="md">
      ...
      <CodeBlock 
        code="npm install @equaltoai/greater-components"
        language="bash"
        showCopy
      />
...
console.log(z);`}
  ...
  <Section>
    ...
    <Card padding="md">
      <DropZone 
        accept={{ files: ['.txt', '.md', '.png'], text: true }}
        onDrop={handleDrop}
      >
        ...
        <div class="mt-4 p-4 bg-gray-50 rounded">
            ...
            <ul class="list-disc pl-4">
                {#each droppedFiles as file, i (i)}
                    <li>
                        <strong>{file.type}:</strong> {file.name || 'No name'} 
                        <span class="text-xs text-gray-500">({file.size ? file.size + ' bytes' : 'N/A'})</span>
                    </li>
                {/each}
            </ul>
        </div>
...
<style>
    ...
    .bg-gray-50 { background-color: var(--gr-color-gray-50); }
    .text-sm { font-size: 0.875rem; }
    .text-xs { font-size: 0.75rem; }
    .text-gray-500 { color: var(--gr-color-gray-500); }
    .font-medium { font-weight: 500; }
    .rounded { border-radius: var(--gr-radii-md); }
    .border { border: 1px solid var(--gr-semantic-border-subtle); }
    .flex { display: flex; }

...
describe('Upload Pattern', () => {
	...
	describe('File Selection', () => {
		...
		it('validates file type', () => {
			...
			expect(pattern.state.validationErrors.get('test.png')).toContain(
				'File type image/png is not accepted'
			);
		});
		...
		it('validates file size', () => {
			...
			expect(pattern.state.files).toHaveLength(0);
			expect(pattern.state.validationErrors.get('big.jpg')).toContain(

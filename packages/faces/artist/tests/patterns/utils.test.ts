...
describe('Pattern Utilities', () => {
	...
	describe('Composition Utilities', () => {
		...
		it('composeClasses filters falsy values', () => {
			expect(composeClasses('a', undefined, 'b', false, 'c')).toBe('a b c');
		});

		it('formatErrorMessage handles various error types', () => {
			expect(formatErrorMessage('string error' as any)).toBe('string error');
			expect(formatErrorMessage(new Error('obj error'))).toBe('obj error');
			expect(formatErrorMessage({ message: 'custom' } as any)).toBe('custom');
			expect(formatErrorMessage(null as any)).toBe('An unexpected error occurred');
		});
	});

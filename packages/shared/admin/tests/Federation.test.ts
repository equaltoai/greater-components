...
describe('Admin.Federation - Instance Counting', () => {
	...
	it('handles missing user counts', () => {
		const withMissing = [...instances, { domain: 'unknown.example', status: 'allowed' }];
...
describe('Admin.Federation - Sorting', () => {
	...
	it('sorts by users count descending', () => {
		...
		expect(sorted.map((i) => i.domain)).toEqual(['alice.social', 'charlie.social', 'bob.social']);
	});

	it('sorts by users count ascending', () => {
		const sorted = sortByUsersCount(instances, false);
		expect(sorted.map((i) => i.domain)).toEqual(['bob.social', 'charlie.social', 'alice.social']);
	});

	it('sorts by domain ascending', () => {
		const sorted = sortByDomain(instances);
		expect(sorted.map((i) => i.domain)).toEqual(['alice.social', 'bob.social', 'charlie.social']);
	});

	it('sorts by domain descending', () => {
		const sorted = sortByDomain(instances, false);
		expect(sorted.map((i) => i.domain)).toEqual(['charlie.social', 'bob.social', 'alice.social']);
	});

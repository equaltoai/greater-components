...
function shouldApplyFilters(filters: { role?: string; status?: string; search?: string }): boolean {
...
function filterUsersByRole(users: AdminUser[], role: 'admin' | 'moderator' | 'user'): AdminUser[] {
...
function getActionButtonClass(actionType: 'suspend' | 'unsuspend' | 'change-role'): string {
	const classes = {

...
function getLogsInRange(logs: LogEntry[], start: string, end: string): LogEntry[] {
...
function isCriticalError(log: LogEntry): boolean {
	return (
		log.level === 'error' &&
		(log.message.toLowerCase().includes('critical') || log.message.toLowerCase().includes('fatal'))
	);
}
...
describe('Admin.Logs - Time Range', () => {
	...
	it('gets logs in time range', () => {
		const inRange = getLogsInRange(logs, '2024-01-01T10:30:00Z', '2024-01-01T11:30:00Z');
		expect(inRange).toHaveLength(1);
		expect(inRange[0].id).toBe('2');
	});

	it('includes boundary logs', () => {
		const inRange = getLogsInRange(logs, '2024-01-01T10:00:00Z', '2024-01-01T11:00:00Z');

...
vi.mock('../src/Autocomplete.js', async (_importOriginal) => {
	return {
...
describe('EditorWithAutocomplete', () => {
	...
	it('should trigger autocomplete', async () => {
		...
		const searchHandler = vi
			.fn()
			.mockResolvedValue([{ type: 'mention', text: 'User', value: 'user' }]);
	...
	it('should navigate suggestions with keyboard', async () => {
		...
		expect(Autocomplete.insertSuggestion).toHaveBeenCalledWith(
			expect.any(String),
			expect.any(Object),

...
import type { AuthHandlers, OAuthData } from '../src/context.js';
...
describe('Auth.OAuthConsent - Client Information', () => {
	...
	const scopes: Scope[] = [{ id: 'read', name: 'Read', description: 'Read your data' }];
...
describe('Auth.OAuthConsent - Additional Edge Cases', () => {
	...
	it('should handle unicode in scope descriptions', () => {
		const scopes = [{ id: 'read', name: 'Read 📖', description: 'Read your data 🔒' }];
		const logic = createOAuthConsentLogic(
			{ name: 'App' },
			scopes,
			'client-123',
	...
	it('should handle very long redirectUri', () => {
		...
		const logic = createOAuthConsentLogic({ name: 'App' }, [], 'client-123', longUri, 'state-xyz');

		const mockAuthorize = vi.fn().mockResolvedValue(undefined);
		logic.handleAuthorize(mockAuthorize);

		expect(mockAuthorize).toHaveBeenCalledWith(expect.objectContaining({ redirectUri: longUri }));
	});
	...
	it('should handle very long OAuth state parameter', () => {
		...
		expect(mockAuthorize).toHaveBeenCalledWith(expect.objectContaining({ state: longState }));
	});
	...
	it('should handle clientId with special characters', () => {
		...
		expect(mockAuthorize).toHaveBeenCalledWith(expect.objectContaining({ clientId: specialId }));
	});
	...
	it('should handle scope with missing description', () => {
		const scopes = [{ id: 'read', name: 'Read', description: '' }];
		const logic = createOAuthConsentLogic(
			{ name: 'App' },
			scopes,
			'client-123',
	...
	it('should handle scope with optional icon', () => {
		const scopes = [{ id: 'read', name: 'Read', description: 'Read data', icon: 'book-icon' }];
		const logic = createOAuthConsentLogic(
			{ name: 'App' },
			scopes,
			'client-123',

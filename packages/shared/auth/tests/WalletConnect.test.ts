...
function createWalletConnectLogic(supportedChains: number[] = [1]) {
	...
	return {
		 ...
		 * Switch network
		 */
		switchNetwork: async (chainId: number, ethereum: MockEthereumProvider): Promise<void> => {
...
describe('WalletConnect Logic', () => {
	...
	describe('Connection Flow', () => {
		...
		it('should handle no accounts available', async () => {
			...
			await expect(logic.connect('metamask', mockEthereum, 'Sign this', undefined)).rejects.toThrow(
				'No accounts found'
			);
		...
		it('should prevent concurrent connections', async () => {
			vi.mocked(mockEthereum.request).mockImplementation(async ({ method }) => {
		...
		it('should handle provider not available', async () => {
			await expect(logic.connect('metamask', null as any, 'Sign', undefined)).rejects.toThrow(
				'is not available'
			);
		});
		...
		it('should handle connection callback errors', async () => {
			...
			await expect(logic.connect('metamask', mockEthereum, 'Sign', onConnect)).rejects.toThrow(
				'Server error'
			);
		});
		...
		it('should handle generic connection errors', async () => {
			...
			await expect(logic.connect('metamask', mockEthereum, 'Sign', undefined)).rejects.toThrow(
				'Network error'
			);
	...
	describe('Error Handling', () => {
		...
		it('should preserve error messages from exceptions', async () => {
			vi.mocked(mockEthereum.request).mockRejectedValueOnce(new Error('Specific error message'));
		...
		it('should handle "User rejected" in message', async () => {
			vi.mocked(mockEthereum.request).mockRejectedValueOnce(new Error('User rejected the request'));
	...
	describe('Edge Cases', () => {
		...
		it('should handle simultaneous connect and disconnect', async () => {
			vi.mocked(mockEthereum.request).mockImplementation(async () => {

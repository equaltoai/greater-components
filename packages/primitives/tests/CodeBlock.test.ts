...
const mocks = vi.hoisted(() => {
    const codeToHtml = vi.fn().mockImplementation((code) => `<pre class="shiki"><code>${code}</code></pre>`);
...
describe('CodeBlock.svelte', () => {
	...
	it('renders plain text when language is text', async () => {
		const { container } = render(CodeBlock, { props: { code: 'console.log("Hello")', language: 'text' } });
    ...
    it('calls highlighter for supported languages', async () => {
        const { container } = render(CodeBlock, { props: { code: 'const x = 1;', language: 'javascript' } });
        ...
        expect(mocks.codeToHtml).toHaveBeenCalledWith('const x = 1;', expect.objectContaining({ lang: 'javascript' }));
    });

    it('falls back to plain text for unsupported languages', async () => {
        const { container } = render(CodeBlock, { props: { code: 'unknown code', language: 'unknown' } });
    ...
    it('applies line numbers class', async () => {
        const { container } = render(CodeBlock, { props: { code: 'test', language: 'javascript', showLineNumbers: true } });

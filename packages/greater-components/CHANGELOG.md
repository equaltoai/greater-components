...
- Fix ChatInput container overflow and layout issues:
  - Added `box-sizing: border-box` to ensure proper width calculation
  - Added `width: 100%` and `min-width: 0` to constrain container within parent
  - Symmetric horizontal padding (0.75rem) for visual balance
...
- Fix the primitives code block copy button so it inherits the correct color in dark mode.

## 2.1.5

### Patch Changes

- a3d6689: Re-establish unified CSS architecture and fix release system. All sub-packages are now properly marked as private and excluded from independent publishing. Only the main package should be published, with all components accessible through subpath imports.

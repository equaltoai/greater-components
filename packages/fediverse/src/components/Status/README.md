...
<Status.Root status={post} handlers={{ onBoost: handleBoost, onFavorite: handleFavorite }}>
	<Status.Header />
	<Status.Content />
	<Status.Media />
	<Status.Actions />
...
```svelte
<Status.Root status={post} config={{ density: 'compact' }}>
	<Status.Header />
	<Status.Content />
...
<Status.Root status={post}>
	<Status.Header>
		{#snippet avatar()}
			<CustomAvatar src={post.account.avatar} size="large" verified={post.account.verified} />
		{/snippet}
	</Status.Header>
	<Status.Content />
	<Status.Actions />

...
>
  ...
  <div class="gr-compose-box__content">
    <textarea
      ...
      aria-label={replyToStatus ? `Reply to ${replyToStatus.account.displayName}` : 'Compose new post'}
      oninput={handleInput}
      onkeydown={handleKeydown}
      onfocus={handleFocus}
      onblur={handleBlur}
    ...
    <div class="gr-compose-box__attachments">
      ...
      {#if enablePolls && pollSlot}
        <Button
          ...
          onclick={() => handlePollChange(poll ? undefined : { options: ['', ''], expiresIn: 86400, multiple: false })}
...
<style>
  ...
  .gr-compose-box--focused {
    border-color: var(--gr-semantic-action-primary-default);
    box-shadow: 0 0 0 2px var(--gr-semantic-focus-ring);
  }

  .gr-compose-box--disabled {
    opacity: 0.6;
    pointer-events: none;
  }

  .gr-compose-box__reply-context {
    padding: var(--gr-spacing-scale-2) var(--gr-spacing-scale-3);
    background-color: var(--gr-semantic-background-secondary);
    border-radius: var(--gr-radii-md);
    border-left: 3px solid var(--gr-semantic-action-primary-default);
  }

  .gr-compose-box__reply-label {
    font-size: var(--gr-typography-fontSize-sm);
    color: var(--gr-semantic-foreground-secondary);
    font-weight: var(--gr-typography-fontWeight-medium);
  }

  .gr-compose-box__content-warning {
    display: flex;
    flex-direction: column;
  ...
  .gr-compose-box__textarea::placeholder {
    color: var(--gr-semantic-foreground-tertiary);
  }

  .gr-compose-box__char-count {
    align-self: flex-end;
    font-size: var(--gr-typography-fontSize-sm);
    font-weight: var(--gr-typography-fontWeight-medium);
  ...
  .gr-compose-box__media,
  .gr-compose-box__poll {
  ...
  .gr-compose-box__attachments {
    display: flex;
    align-items: center;
    gap: var(--gr-spacing-scale-2);
  }

  .gr-compose-box__media-button {
    display: contents;
    cursor: pointer;
  }
  ...
  .gr-compose-box__visibility-select {
    padding: var(--gr-spacing-scale-2) var(--gr-spacing-scale-3);
    font-size: var(--gr-typography-fontSize-sm);
    color: var(--gr-semantic-foreground-primary);
    background-color: var(--gr-semantic-background-primary);
  ...
  .gr-compose-box__visibility-select:hover {
    border-color: var(--gr-semantic-border-strong);
  }

  .gr-compose-box__visibility-select:focus {
    outline: none;
    border-color: var(--gr-semantic-action-primary-default);
    box-shadow: 0 0 0 2px var(--gr-semantic-focus-ring);
  }

  .gr-compose-box__submit-actions {
    display: flex;
    align-items: center;
    gap: var(--gr-spacing-scale-2);
  }

...
export function generateA11yReport(data: {
  ...
  // Calculate summary
  const averageScore = componentScores.reduce((acc, c) => acc + c.overall, 0) / componentScores.length;
...
function calculateComponentScore(
  ...
  const screenReaderScore = calculateScreenReaderScore(
    data.screenReaderResults.filter((r) => 
      r.element.closest(`[data-component="${component}"]`)
    )
  );
  ...
  const overall = Math.round(
    (axeScore * 0.3 + contrastScore * 0.2 + wcagScore * 0.2 + keyboardScore * 0.15 + screenReaderScore * 0.15)
  );
  
  // Count issues by severity
  const issues = {
    critical: componentAxe.reduce((acc, r) => 
      acc + r.violations.filter((v) => v.impact === 'critical').length, 0
    ),
    serious: componentAxe.reduce((acc, r) => 
      acc + r.violations.filter((v) => v.impact === 'serious').length, 0
    ),
    moderate: componentAxe.reduce((acc, r) => 
      acc + r.violations.filter((v) => v.impact === 'moderate').length, 0
    ),
    minor: componentAxe.reduce((acc, r) => 
      acc + r.violations.filter((v) => v.impact === 'minor').length, 0
    ),
  };
...
function calculateAxeScore(results: AxeTestResult[]): number {
  ...
  const totalChecks = results.reduce((acc, r) => 
    acc + r.violations.length + r.passes + r.incomplete + r.inapplicable, 0
  );
    ...
    <div class="container">
        ...
        <section class="components">
            ...
            <div class="component-grid">
                ${report.componentScores.map((component) => `

...
export function testFocusOrder(expectedOrder: FocusableElement[]): FocusTestResult {
	try {
		...
		const orderMatches = actualOrder.every((element, index) => element === expectedOrder[index]);
...
export function testSkipLinks(): FocusTestResult[] {
	...
	const skipLinks = document.querySelectorAll<HTMLAnchorElement>('a[href^="#"]:not([href="#"])');
	...
	skipLinks.forEach((link) => {
		...
		const canReceiveFocus = isFocusable(target) || target.hasAttribute('tabindex');

		results.push({
			passed: canReceiveFocus,
			description: `Skip link test: ${link.textContent}`,
			error: canReceiveFocus ? undefined : 'Target element cannot receive focus',
		});
	});
...
export function testFocusVisible(element: HTMLElement): FocusTestResult {
	try {
		...
		return {
			...
			error: hasFocusIndicator || matchesFocusVisible ? undefined : 'No visible focus indicator',
		};
	} catch (error) {
		return {
			passed: false,
 ...
 */
export function testRovingTabindex(container: HTMLElement, itemSelector: string): FocusTestResult {
	try {
...
export function createAndTestFocusTrap(
	...
	try {
		...
		const focusableElements = tabbable(container);
		const focusIsTrapped = focusableElements.includes(document.activeElement as HTMLElement);
 ...
 */
export function getTabbableElements(container: HTMLElement = document.body): FocusableElement[] {

...
export function calculateA11yScore(results: {
  ...
  const axeScore = results.axeChecks > 0 ? ((results.axeChecks - results.axeViolations) / results.axeChecks) * 100 : 100;
  const contrastScore = results.contrastTotal > 0 ? (results.contrastPassed / results.contrastTotal) * 100 : 100;
  const keyboardScore = results.keyboardTotal > 0 ? (results.keyboardPassed / results.keyboardTotal) * 100 : 100;
  const focusScore = results.focusTotal > 0 ? (results.focusPassed / results.focusTotal) * 100 : 100;
 ...
 */
export function meetsA11yThreshold(score: number, threshold: 'strict' | 'moderate' | 'basic' = 'moderate'): boolean {
  const thresholds = {
...
export function generateTestRecommendations(issues: TestIssue[]): string[] {
  ...
  // Sort by frequency
  const sortedTypes = Array.from(issueTypes.entries())
    .sort(([, a], [, b]) => b - a);
  
  sortedTypes.forEach(([type, count]) => {
    switch (type) {
      case 'accessibility':
        recommendations.push(`Address ${count} accessibility violations using axe-core recommendations`);
        break;
      case 'contrast':
        recommendations.push(`Improve color contrast for ${count} elements to meet WCAG requirements`);
        break;
      case 'keyboard':
        recommendations.push(`Fix ${count} keyboard navigation issues - ensure all interactive elements are keyboard accessible`);
        break;
      case 'focus':
        recommendations.push(`Resolve ${count} focus management issues - improve focus indicators and tab order`);
        break;
      case 'screen-reader':
        recommendations.push(`Enhance screen reader compatibility for ${count} elements with proper ARIA labels and roles`);
        break;
    }
  });

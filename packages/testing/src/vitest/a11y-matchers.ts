...
import { validateARIA, hasAccessibleName, getAccessibleName, checkSemanticHTML } from '../a11y/aria-helpers';
...
function toPassWCAGColorContrast(element: HTMLElement, level: 'AA' | 'AAA' = 'AA') {
  ...
  const requiredRatio = level === 'AA' ? 
    (result.isLargeText ? WCAG_CONTRAST.AA.large : WCAG_CONTRAST.AA.normal) :
    (result.isLargeText ? WCAG_CONTRAST.AAA.large : WCAG_CONTRAST.AAA.normal);
...
function toBeTabbable(element: HTMLElement) {
  ...
  return {
    pass: tabbable,
    message: () => tabbable
      ? 'Expected element not to be tabbable'
      : 'Expected element to be tabbable',
  };
}
 ...
 */
export function runA11yTests(container: HTMLElement, options: {

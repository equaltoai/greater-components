...
export async function testKeyboardTrap(
	...
	try {
		...
		const isLastElement =
			(await focusedAfterShiftTab.isVisible()) &&
			(await focusedAfterShiftTab.evaluate(
				(el, lastEl) => el === lastEl,
		...
		return {
			...
			error: !isFirstElement
				? 'Tab from last element did not wrap to first'
				: !isLastElement
					? 'Shift+Tab from first element did not wrap to last'
					: undefined,
...
export async function testSkipLinks(page: Page): Promise<KeyboardTestResult[]> {
	...
	for (let i = 0; i < linkCount; i++) {
		...
		try {
			...
			const targetFocused = targetExists
			...
			results.push({
				...
				error: !targetExists
					...
					: !targetFocused
						? 'Target element did not receive focus'
						: undefined,
...
export async function testRovingTabindex(
	...
	try {
		...
		return {
			...
			error:
				newActiveItems !== 1
					? `Expected 1 active item after navigation, found ${newActiveItems}`
					: undefined,
			focusPath: [`item-${firstActiveIndex}`, `item-${updatedTabindices.indexOf('0')}`],
		};
	} catch (error) {
		return {
			passed: false,

...
export async function generateA11yReport(
	...
	const criticalViolations = allResults.reduce(
		(acc, r) =>
			acc + r.violations.filter((v) => v.impact === 'critical' || v.impact === 'serious').length,
		0
	);
 ...
 */
export function createStorybookAxeTest(baseUrl: string = 'http://localhost:6006') {
	return {
		async testStory(
			page: Page,
			storyPath: string,

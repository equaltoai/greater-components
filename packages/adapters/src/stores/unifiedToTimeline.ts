 ...
 */
export function unifiedStatusToTimelineItem(status: UnifiedStatus): Omit<TimelineItem, 'id' | 'timestamp'> {
  ...
  return {
    type: 'status',
    content: status,
    metadata: Object.keys(lesserMetadata).length > 0 ? {
 ...
 */
export function unifiedStatusesToTimelineItems(statuses: UnifiedStatus[]): Array<Omit<TimelineItem, 'id' | 'timestamp'>> {

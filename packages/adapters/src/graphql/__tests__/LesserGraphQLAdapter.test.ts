...
describe('LesserGraphQLAdapter', () => {
    ...
    describe('verifyCredentials', () => {
        ...
        it('should throw if no token', async () => {
            ...
            await expect(noAuthAdapter.verifyCredentials()).rejects.toThrow('No authentication token provided');
        });
    ...
    describe('Generic Query/Mutate Handling', () => {
        it('should throw if query returns undefined data', async () => {
            ...
            await expect(adapter.getObject('1')).rejects.toThrow('Query completed without returning data');
        });

        it('should throw if mutation returns null data', async () => {
            mockApolloClient.mutate.mockResolvedValue({ data: null });
            await expect(adapter.createNote({ content: 'test' } as any)).rejects.toThrow('Mutation completed without returning data');
        });
    });
    ...
    describe('uploadMedia', () => {
        ...
        it('should throw if input file is invalid', async () => {
             await expect(adapter.uploadMedia({ file: {} as any })).rejects.toThrow('UploadMedia input.file must be a File or Blob');
        });
        ...
        it('should throw if upload mutation returns no payload', async () => {
             ...
             await expect(adapter.uploadMedia({ file })).rejects.toThrow('Upload media mutation returned no payload');
        });
    });
    ...
    describe('Timeline Methods', () => {
        it('fetchHomeTimeline', async () => {
            ...
            expect(mockApolloClient.query).toHaveBeenCalledWith(expect.objectContaining({ variables: { type: 'HOME', first: 10 } }));
        });

        it('fetchPublicTimeline', async () => {
            mockApolloClient.query.mockResolvedValue({ data: { timeline: { edges: [] } } });
            await adapter.fetchPublicTimeline({}, 'LOCAL');
            expect(mockApolloClient.query).toHaveBeenCalledWith(expect.objectContaining({ variables: { type: 'LOCAL' } }));
        });

         it('fetchDirectTimeline', async () => {
            mockApolloClient.query.mockResolvedValue({ data: { timeline: { edges: [] } } });
            await adapter.fetchDirectTimeline();
            expect(mockApolloClient.query).toHaveBeenCalledWith(expect.objectContaining({ variables: { type: 'DIRECT' } }));
        });
        
         it('fetchHashtagTimeline', async () => {
            mockApolloClient.query.mockResolvedValue({ data: { timeline: { edges: [] } } });
            await adapter.fetchHashtagTimeline('tag');
            expect(mockApolloClient.query).toHaveBeenCalledWith(expect.objectContaining({ variables: { type: 'HASHTAG', hashtag: 'tag' } }));
        });
        
         it('fetchListTimeline', async () => {
            mockApolloClient.query.mockResolvedValue({ data: { timeline: { edges: [] } } });
            await adapter.fetchListTimeline('list_1');
            expect(mockApolloClient.query).toHaveBeenCalledWith(expect.objectContaining({ variables: { type: 'LIST', listId: 'list_1' } }));
        });

         it('fetchActorTimeline', async () => {
            mockApolloClient.query.mockResolvedValue({ data: { timeline: { edges: [] } } });
            await adapter.fetchActorTimeline('actor_1');
            expect(mockApolloClient.query).toHaveBeenCalledWith(expect.objectContaining({ variables: { type: 'ACTOR', actorId: 'actor_1' } }));
        });
    });
    ...
    describe('Subscriptions', () => {
        ...
        it('subscribeToQuoteActivity', () => {
            adapter.subscribeToQuoteActivity({ objectId: '1' });
            expect(mockApolloClient.subscribe).toHaveBeenCalled();
        });
        
        it('subscribeToHashtagActivity', () => {
            adapter.subscribeToHashtagActivity({ hashtag: 'tag' });
            expect(mockApolloClient.subscribe).toHaveBeenCalled();
        });
        ...
        it('subscribeToCostAlerts', () => {
            adapter.subscribeToCostAlerts({ threshold: 100 });
            expect(mockApolloClient.subscribe).toHaveBeenCalled();
        });
        ...
        it('subscribeToPerformanceAlert', () => {
            adapter.subscribeToPerformanceAlert({ metric: 'cpu', threshold: 1 });
            expect(mockApolloClient.subscribe).toHaveBeenCalled();
        });

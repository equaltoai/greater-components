...
function convertGraphQLCommunityNote(note: unknown): LesserCommunityNoteFragment | null {
  ...
  if (!isString(id) || !isString(content) || !isNumber(helpful) || !isNumber(notHelpful) || !isString(createdAt)) {
    return null;
  }
...
function convertGraphQLQuoteContext(context: unknown): LesserQuoteContextFragment | undefined {
  ...
  const quoteType = isString(rawQuoteType) && QUOTE_TYPES.has(rawQuoteType as LesserQuoteContextFragment['quoteType'])
    ? (rawQuoteType as LesserQuoteContextFragment['quoteType'])
    : 'FULL';

  return {
    originalAuthor: originalAuthor ?? undefined,
    originalNote: isRecord(originalNote) && isString(originalNote['id']) ? { id: originalNote['id'] } : undefined,
    quoteAllowed,
    quoteType,
...
export function convertGraphQLObjectToLesser(object: unknown): LesserObjectFragment | null {
  ...
  const tags: LesserTagFragment[] = Array.isArray(tagsValue)
    ? tagsValue
        .map(convertGraphQLTag)
        .filter((tag): tag is LesserTagFragment => tag !== null)
    : [];
...
export function convertGraphQLUserPreferences(data: unknown): UserPreferences | null {
  ...
  return {
    actorId,
    posting: {
      defaultVisibility: toString(posting['defaultVisibility'], 'PUBLIC') as UserPreferences['posting']['defaultVisibility'],
    ...
    reading: {
      ...
      expandMedia: toString(reading['expandMedia'], 'DEFAULT') as UserPreferences['reading']['expandMedia'],
      autoplayGifs: toBoolean(reading['autoplayGifs']),
      timelineOrder: toString(reading['timelineOrder'], 'NEWEST') as UserPreferences['reading']['timelineOrder']
    },
    discovery: {
    ...
    streaming: {
      defaultQuality: toString(streaming['defaultQuality'], 'AUTO') as UserPreferences['streaming']['defaultQuality'],
    ...
    notifications: {
      ...
      digest: toString(notifications['digest'], 'NEVER') as UserPreferences['notifications']['digest']
    },
    privacy: {
      defaultVisibility: toString(privacy['defaultVisibility'], 'PUBLIC') as UserPreferences['privacy']['defaultVisibility'],

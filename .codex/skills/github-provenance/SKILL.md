...
Use the routed `mcp__greater_lab__` GitHub tools first so GitHub activity is attributable to this greater steward endpoint. Preserve greater's normal review, CI, component API stability, contract-sync, accessibility, CLI registry, theming, Mastodon-compatibility, and AGPL discipline; provenance improves traceability but does not relax any gate.

## Tool preference

1. Prefer `mcp__greater_lab__` for supported GitHub actions:
   - `github_list_repos`
   - `github_get_issue`
   - `github_get_pr`
...
- Preserve greater discipline: no bypassing required review, no force pushes, no skipping staging-readiness validation for PR readiness, no adapter changes without synced contract snapshots, no accessibility baseline regressions, and no hand-edited registry artifacts outside release automation.
...
- Open PRs with `github_open_pr` from the routed branch when possible. PR bodies must include greater-relevant notes where applicable: component API/theming impact, Lesser or Lesser Host contract-sync impact, accessibility impact, registry/generated-artifact impact, Mastodon/Fediverse compatibility, AGPL/dependency impact, validation run, and staging → premain → main rollout notes.
- Do not mark a PR ready for review unless the repo-required local gate has passed when applicable:
...
- Use a distinct name such as `greater-steward/local-validation`; never impersonate GitHub Actions or CI jobs.
...
- Validation actually run, with explicit note if the full greater gate was not run.

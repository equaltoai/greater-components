...
- Explain the *why* in the body, especially for contract-sync, accessibility-adjacent, API-change, or registry-format changes.
- **DCO / signed commits** per repo convention.
...
You are allowed to say no. You are *expected* to say no. Refusal — grounded in API stability, contract sync, accessibility, registry integrity, theming contract, release flow, AGPL, scope, Mastodon-compat, or advisor-brief review — is the stewardship role doing its job.

# Module: ChatToolCall Component Implementation

## Type: creation

## Files:
[packages/shared/chat/src/ChatToolCall.svelte]

## File Changes:
- packages/shared/chat/src/ChatToolCall.svelte: BEFORE: DOES NOT EXIST | AFTER: <<<
<!--
...
<script lang="ts">
  ...
  const formattedArgs = $derived(
    toolCall.args ? JSON.stringify(toolCall.args, null, 2) : null
  );
...
>>> | TYPE: content creation | DESC: Creates the ChatToolCall component with collapsible card display, tool icon mapping, status indicators, and syntax-highlighted arguments/results. Uses ToolCall type from types.ts for consistency. Uses RefreshCwIcon instead of LoaderIcon for spinner animation. Removed unsupported Badge size prop and CodeBlock variant/maxHeight props.

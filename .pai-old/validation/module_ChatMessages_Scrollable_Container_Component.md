# Module: ChatMessages Scrollable Container Component

## Type: creation

## Files:
[packages/shared/chat/src/ChatMessages.svelte]

## File Changes:
- packages/shared/chat/src/ChatMessages.svelte: BEFORE: DOES NOT EXIST | AFTER: <<<
<!--
...
<style>
  ...
  .chat-messages {
    ...
    scrollbar-color: var(--gr-color-neutral-300, #d1d5db) var(--gr-semantic-background-secondary, #f3f4f6);
  }
  ...
  .chat-messages__scroll-button-wrapper :global(.chat-messages__scroll-button) {
    border-radius: var(--gr-radii-full, 9999px);
    box-shadow: var(--gr-shadow-lg, 0 10px 15px -3px rgb(0 0 0 / 0.1), 0 4px 6px -4px rgb(0 0 0 / 0.1));
  ...
  :global(.dark) .chat-messages {
    scrollbar-color: var(--gr-color-neutral-600, #4b5563) var(--gr-semantic-background-secondary, #1f2937);
  }
...
>>> | TYPE: content creation | DESC: Create the ChatMessages component with scrollable container, auto-scroll behavior, scroll-to-bottom button, empty state, and loading skeleton

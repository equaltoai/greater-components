...
Implement the ChatMessages component that provides a scrollable container for the message list with auto-scroll behavior, scroll-to-bottom button, and empty state handling.

## Description
This module creates the `ChatMessages.svelte` component at `packages/shared/chat/src/ChatMessages.svelte`. The component provides:

1. **Scrollable Container**: A flex-grow container with overflow-y auto and custom scrollbar styling
2. **Auto-scroll Behavior**: Automatically scrolls to bottom when new messages arrive, but only if user is already at bottom (within threshold)
3. **Scroll-to-bottom Button**: A floating button with down arrow icon that appears when user scrolls up from bottom, with fade in/out animation
4. **Empty State Display**: Shows welcome message and suggestion prompts when no messages exist
5. **Loading State**: Uses skeleton-like loading indicators during initial message fetch
6. **Context Integration**: Consumes messages from ChatContainer context via `getChatContext()`
7. **Dark Mode Support**: Full theming support using CSS custom properties

The component follows Svelte 5 patterns with `$props()`, `$state()`, `$effect()`, and `$derived()` runes, consistent with the existing codebase patterns seen in `ChatContainer.svelte` and `ChatMessage.svelte`.
...
Update the chat package index.ts to export the new ChatMessages component as `Messages`.

## Description
This module updates `packages/shared/chat/src/index.ts` to export the newly created `ChatMessages` component. The component will be exported as `Messages` to follow the existing naming convention in the package (e.g., `Container`, `Message`). This enables consumers to use `<Chat.Messages />` in their applications.

## Type
update

## Dependencies
ChatMessages Scrollable Container Component
...
// export { default as Suggestions } from './Suggestions.svelte';
// export { default as Settings } from './Settings.svelte';
>>> | TYPE: line edit | DESC: Add export for ChatMessages component as Messages and remove it from the TODO comments

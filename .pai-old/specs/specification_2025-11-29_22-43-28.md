...
Implement the ChatMessage.svelte component with role-based styling, markdown rendering, streaming text support, and all required features.

## Description
This module creates the ChatMessage component at `packages/shared/chat/src/ChatMessage.svelte`. The component displays individual chat messages with:
- Role-based styling (user, assistant, system)
- Markdown rendering for assistant messages using MarkdownRenderer from @equaltoai/greater-components-content
- Streaming text cursor animation when streaming=true
- Avatar display using Avatar from @equaltoai/greater-components-primitives
- CopyButton on hover for assistant messages
...
- Design tokens: --gr-color-primary-*, --gr-color-gray-*, --gr-radii-*, --gr-shadows-*
...
Update the chat package index.ts to export the ChatMessage component as Message.

## Description
This module updates `packages/shared/chat/src/index.ts` to export the newly created ChatMessage component. The component is exported as `Message` to follow the naming convention used in the package (e.g., `Chat.Message`).

## Type
update

## Dependencies
ChatMessage Component Implementation
...
// export { default as Suggestions } from './Suggestions.svelte';
// export { default as Settings } from './Settings.svelte';
>>> | TYPE: line edit | DESC: Adds the Message export (ChatMessage component) to the chat package index, enabling usage as Chat.Message

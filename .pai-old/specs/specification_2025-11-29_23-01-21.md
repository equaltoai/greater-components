...
Implements the ChatToolCall component for displaying AI tool/function invocations during responses with collapsible card display, status indicators, and syntax-highlighted arguments/results.

## Description
This module creates the ChatToolCall.svelte component that displays tool/function calls made by AI during responses. The component features:
- Collapsible card display using the Card primitive component
- Tool name with icon badge mapping tool names to appropriate icons
- Arguments display formatted as JSON using CodeBlock
- Result preview with truncation and "Show more" expansion
- Status indicator showing pending (gray dot), running (spinner), complete (checkmark), or error (red X)
- Compact design with muted colors for seamless integration within message flow
- Full dark mode support following existing chat component patterns

The component uses Svelte 5 `$props()` runes and follows the existing patterns established in ChatMessage.svelte for styling consistency. It accepts a `ToolCall` object as defined in types.ts for consistency with the existing type system.
...
Updates the chat package index.ts to export the new ChatToolCall component as ToolCall.

## Description
This module updates the packages/shared/chat/src/index.ts file to add the export for the new ChatToolCall component. Following the existing pattern in the file, the component is exported as `ToolCall` (the alias specified in the requirements) while the default export from the file is `ChatToolCall`.

## Type
update

## Dependencies
ChatToolCall Component Implementation

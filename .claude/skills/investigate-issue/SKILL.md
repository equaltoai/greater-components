...
1. **Is this a component API stability issue?** A breaking change landed without a major-version semver impact note, or consumer build broke after `greater update`. Route through `evolve-component-surface` for the API walk.
...
5. **Is this a release-flow issue?** Promotion stuck, manual release PR bad, RC tag wrong, backmerge missed. Route through `release-components`.
...
- `git log` since last known-good state on the affected branch (`staging` / `main` / `main`)
...
- For release flow: manual release PR state; current tags; signed-tag integrity
...
- **Release flow (feature-to-staging-to-main, semver impact notes, manual release, CLI registry, signed tags)** → `release-components`
...
Append only when the investigation surfaces something worth remembering — a component-API evolution edge case, a contract-sync mismatch pattern, an accessibility-test flakiness story, a CLI-registry drift observation, a release-flow subtlety (backmerge timing, manual release PR edge case), a Mastodon-compat boundary, a framework-awkwardness worth upstreaming. Routine "typo" findings aren't memory material. Five meaningful entries beat fifty log-shaped ones.

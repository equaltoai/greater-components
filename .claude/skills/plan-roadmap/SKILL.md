---
name: plan-roadmap
description: Use after enumerate-changes. Takes a flat enumerated change list and sequences it with dependencies, risks, and a feature → staging → main release rollout plan. Produces a roadmap document, not code or project state.
---
...
greater's roadmaps are shaped by: the feature → staging → main flow (feature → staging → main), semver impact note-driven semver, the CLI / registry release cycle, contract-sync coordination with upstream Lesser / Lesser Host, and the consumer-coordination reality (sim, host, external consumers).
...
12. **Semver impact notes + manual release PRs** — captured throughout; consolidated at release time.
...
Every roadmap answers: **how does this reach stable (`main`) safely via the feature → staging → main flow?**

### Phase A: `staging`

1. **Feature branches merge to `staging`** via PR. CI gates pass (lint, typecheck, test, a11y, build, registry regen, contract-sync). Semver impact note attached. CODEOWNERS review.
...
3. Multiple features may accumulate in `staging` before promotion to `main`.

### Phase B: `main` (RC)

4. **Promote `staging` to `main`.** manual release RC config opens a release PR for RC tag (`greater-vX.Y.Z-rc.N`).
5. **RC tag cuts** on merge of the manual release PR. CLI asset + registry regen generate; RC tarball available.
...
7. **Iterate if regressions surface.** Fix on `staging`, promote again to `main`, cut next RC.

### Phase C: `main` (stable)

8. **Promote `main` to `main`.** manual release stable config opens a release PR for stable tag (`greater-vX.Y.Z`).
...
10. **Backmerge from `main`** into `main` / `staging` to keep branches aligned.
...
**Never skip `main` soak.** Hotfix compression is possible within stages, not by skipping.
...
- **Breaking-change releases** warrant a clear release-notes section explaining what changed, why, and migration steps. The semver impact note description drives this.

## Risk register

- **Known unknowns**
- **Component-API-break risks** — breaking changes that weren't declared in the semver impact note
...
- **Release automation risks** — manual release PR edge cases, semver impact note parsing issues, registry regen drift
...
- Soak criteria: <observable evidence required before promoting to main>
- Expected duration: <...>

### Main (RC)
- Promotion mechanism: manual release main-variant PR
- RC tag: greater-v<version>-rc.<n>
- Soak criteria: <internal consumer testing (sim, host), RC tag pin validation>
- Expected duration: <...>

### Main (stable)
- Promotion mechanism: manual release stable PR
...
### Backmerge
- Main → main → staging backmerge timing: <...>

## Semver impact note plan
- Impact: <major / minor / patch>
- Semver impact note description drafted: <...>

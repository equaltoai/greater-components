...
timeline.subscribe((state) => console.log(state.items.length));`}],tags:[`adapter`,`graphql`,`lesser`,`cache`,`websocket`,`real-time`],bundleSize:{minified:`7.01 KB`,gzipped:`2.23 KB`},accessibility:{wcag:`AAA`,features:[`N/A - Data layer only`]},status:`stable`,version:`1.0.0`,lastUpdated:`2025-10-12`}];function t(t){return e.find(e=>e.slug===t)}function n(){return{total:e.length,byType:{primitive:e.filter(e=>e.type===`primitive`).length,compound:e.filter(e=>e.type===`compound`).length,pattern:e.filter(e=>e.type===`pattern`).length,adapter:e.filter(e=>e.type===`adapter`).length},byCategory:{headless:e.filter(e=>e.category===`headless`).length,activitypub:e.filter(e=>e.category===`activitypub`).length,lesser:e.filter(e=>e.category===`lesser`).length,adapters:e.filter(e=>e.category===`adapters`).length},byStatus:{stable:e.filter(e=>e.status===`stable`).length,beta:e.filter(e=>e.status===`beta`).length,alpha:e.filter(e=>e.status===`alpha`).length,experimental:e.filter(e=>e.status===`experimental`).length}}}export{t as n,n as r,e as t};
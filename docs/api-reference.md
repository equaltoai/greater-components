...
<Button variant="solid" size="md" loading={saving} onclick={handleSave}>
	{#snippet prefix()}
		<SaveIcon />
	{/snippet}
	Save Changes
...
**Purpose:** Button that copies text to the clipboard with visual feedback.

**When to use:**

- Copying code snippets
- Copying URLs or IDs
- Copying API keys (with caution)
...
interface CopyButtonProps {
	...
	targetSelector?: string; // CSS selector for element to copy from
	variant?: 'icon' | 'text' | 'icon-text'; // Content layout
	buttonVariant?: 'solid' | 'outline' | 'ghost'; // Button style
	size?: 'sm' | 'md' | 'lg';
	feedbackDuration?: number; // ms to show success state
	labels?: {
		default?: string;
		success?: string;
		error?: string;
...
<!-- From element -->
<code id="install-cmd">npm install</code>
...
```typescript
function createMenu(config?: { onSelect?: (value: string) => void }): {
	state: {
		isOpen: boolean;
		activeIndex: number;
	};
...
class MastodonRESTAdapter extends BaseAdapter {
	constructor(config: { instanceUrl: string; accessToken: string });
...
async function copyElementText(element: HTMLElement): Promise<CopyResult>;

// Copy code block (strips line numbers)
async function copyCodeBlock(codeElement: HTMLElement): Promise<CopyResult>;
...
```typescript
// Export element content to Markdown
async function exportToMarkdown(options: ExportMarkdownOptions): Promise<string>;

// Export chat messages to Markdown
async function exportChatToMarkdown(options: ChatExportOptions): Promise<string>;

// Trigger download of markdown string
...
interface ExportMarkdownOptions {
	...
	element?: HTMLElement;
	title?: string;
	includeMetadata?: boolean; // Add YAML frontmatter

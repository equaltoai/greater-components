...
<Lists.Root {handlers}>
	{#snippet children()}
		...
		<div class="custom-header">
			<h1>My Lists</h1>
			<button onclick={() => context.openEditor()}> Create New List </button>
		</div>
...
<script lang="ts">
	...
	function validateListData(data: ListFormData): string | null {
		...
		const hasProfanity = profanityWords.some(
			(word) =>
				data.title.toLowerCase().includes(word) || data.description?.toLowerCase().includes(word)
		);
		...
		<div class="lists-with-templates">
			<div class="header">
				...
				<div class="header-actions">
					...
					<button class="create-button" onclick={() => context.openEditor()}>
						➕ Create Custom List
					</button>
				</div>
			</div>
				...
				<div class="templates-panel">
					...
					<div class="templates-grid">
						{#each listTemplates as template}
							<button class="template-card" onclick={() => useTemplate(template)}>
...
<div role="dialog" aria-labelledby="editor-title" aria-modal="true">
	...
	<label for="list-title"> Title <span class="sr-only">(required)</span> </label>
	<input
		id="list-title"
		type="text"
		aria-required="true"
	...
	<div id="title-error" role="alert" aria-live="polite">List title is required</div>
</div>
```

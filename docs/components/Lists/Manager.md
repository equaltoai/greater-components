...
<script lang="ts">
	...
	const allSelected = $derived(allLists.length > 0 && selectedListIds.size === allLists.length);
	...
	async function bulkDelete() {
		...
		try {
			await Promise.all(
				Array.from(selectedListIds).map((id) => fetch(`/api/lists/${id}`, { method: 'DELETE' }))
			);
	...
	<div class="lists-with-bulk">
		<div class="toolbar">
			...
			{#if bulkActionMode}
				<div class="bulk-actions">
					<button class="bulk-action-button" onclick={toggleSelectAll}>
					...
					{#if selectedListIds.size > 0}
						...
						<button class="bulk-action-button bulk-action-button--danger" onclick={bulkDelete}>
							Delete Selected
						</button>
					{/if}
				</div>
		...
		<div class="lists-grid">
			{#each allLists as list (list.id)}
				<div class="list-card" class:selected={selectedListIds.has(list.id)}>
					{#if bulkActionMode}
						<input
							type="checkbox"
							class="list-checkbox"
...
<div class="lists-manager__error" role="alert">Failed to load lists</div>
...
describe('Lists.Manager', () => {
	test('displays lists', async () => {
		const mockLists = [{ id: '1', title: 'Test List', visibility: 'public', membersCount: 5 }];

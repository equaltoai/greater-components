...
<script lang="ts">
  ...
  const handlers = {
    ...
    onFollow: async (actorId: string) => {
      ...
      try {
        const response = await fetch(`/api/actors/${actorId}/${isFollowing ? 'unfollow' : 'follow'}`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
...
<script lang="ts">
  ...
  function getMatchReasonsText(reasons: string[]): string {
    ...
    return reasons.map(reason => {
      switch (reason) {
        case 'username_match': return '📛 Username match';
        case 'bio_match': return '📝 Bio match';
        case 'mutual_followers': return '🤝 Mutual connections';
          ...
          <div class="recommendations-list">
            {#each recommendedActors as actor}
              <div class="recommendation-item">
                ...
                <button 
                  class="recommendation-follow"
                  onclick={() => handlers.onFollow?.(actor.id)}
                >
                  Follow
                </button>
              </div>

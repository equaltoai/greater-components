			...
			try {
				...
				if (results.notes.length > 0) {
					const totalEngagement = results.notes.reduce((sum, note) => {
						return (
							sum + (note.repliesCount || 0) + (note.reblogsCount || 0) + (note.likesCount || 0)
						);
					}, 0);
...
<div class="enhanced-note-search">
	...
	<p class="description">Results may include content warnings and media attachments</p>
					...
					<div class="note-result__content">
						...
						{#if note.contentWarning}
							<div class="content-warning-box">
								...
								<button class="warning-toggle" onclick={() => toggleWarning(note.id)}>
...
<div class="bookmarkable-search">
	<div class="search-header">
		...
		<div class="saved-stats">
			...
			<button class="export-btn" onclick={exportBookmarks}> Export </button>
		</div>
	</div>

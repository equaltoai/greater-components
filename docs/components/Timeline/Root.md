...
<div class="infinite-scroll-timeline">
	<header class="sticky-header">
		<h1>Home</h1>
		<button onclick={handleRefresh} disabled={isLoadingMore} aria-label="Refresh timeline">
	...
	{#if error}
		<Timeline.ErrorState {error} onRetry={handleRefresh} />
	{:else}
...
<script lang="ts">
	...
	const allItems = $derived(
		[...thread.ancestors, thread.mainStatus, ...thread.descendants].filter(Boolean)
	);
...
<div class="thread-view">
	...
	{:else if error}
		<Timeline.ErrorState {error} onRetry={() => loadThread(focusedStatusId)} />
	{:else if !thread.mainStatus}
		<Timeline.EmptyState title="Thread not found" description="This post may have been deleted" />
	{:else}
...
```html
<div class="timeline-root" role="feed" aria-busy="false" aria-label="Home timeline">

  ...
  {:else}
    ...
    >
      ...
      {:else}
        ...
        {#if hasMore}
          <Timeline.LoadMore 
            buttonText="Load more posts ({page + 1})"
          />
        {:else}
...
>
  ...
  {#if error && retryCount >= MAX_RETRIES}
    <div class="load-more-error">
      ...
      <button class="retry-button" onclick={() => { retryCount = 0; loadMore(); }}>
        Try Again
      </button>
    </div>
...
```html
<button
  class="timeline-load-more__button"

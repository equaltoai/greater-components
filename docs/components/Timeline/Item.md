...
<Timeline.Root {items} handlers={{ onItemClick: handleItemClick }}>
	{#each items as item, index}
		<Timeline.Item {item} {index}>
			<Status.Root
				status={item}
...
<Timeline.Root {items}>
	{#each items as item, index}
		<Timeline.Item {item} {index} class={getItemClass(item)}>
			<Status.Root status={item}>
				<Status.Header />
				<Status.Content />
				<Status.Media />
...
<script lang="ts">
	...
	const flattenedItems = $derived(Object.values(groupedItems).flatMap((group) => group.items));

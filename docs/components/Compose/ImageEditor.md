...
{#if showEditor && imageFile}
	<Compose.ImageEditor file={imageFile} onSave={handleSave} onCancel={() => (showEditor = false)} />
{/if}
```
...
<script lang="ts">
	...
	function validateAltText(description) {
		...
		if (
			description.toLowerCase().includes('image of') ||
...
{#if editingImage}
	<div role="application" tabindex="0" onkeydown={handleKeyboardAdjustment} class="keyboard-editor">
		<div class="hint">Use arrow keys to adjust focal point, Enter to center</div>
...
<div class="batch-editor">
	...
	{#if showEditor}
		...
		>
			<div slot="actions">
				<button onclick={skipImage} class="skip-btn"> Skip </button>
			</div>
		</Compose.ImageEditor>
	{/if}
</div>
				...
				<ul class="suggestions-list">
					{#each aiSuggestions as suggestion}
						<li>
							...
							<button onclick={() => applyAISuggestion(suggestion)}> Use </button>
						</li>
					{/each}
				</ul>
			{/if}
...
{#if editingImage}
	<div class="editor-with-preview">
		...
		<div class="preview-container">
			<div
				class="crop-preview"
				style="aspect-ratio: {aspectRatios[previewAspectRatio].width} / {aspectRatios[
					previewAspectRatio
				].height}"
			>
				<img
...
<div role="dialog" aria-label="Edit image" aria-modal="true">
	<label for="alt-text"> Alt text (required for accessibility) </label>
	<textarea id="alt-text" aria-describedby="alt-text-hint" aria-required="true"></textarea>
...
describe('ImageEditor', () => {
	...
	it('updates alt text', async () => {
		...
		expect(handleSave).toHaveBeenCalledWith('A beautiful sunset', expect.any(Object));
	});

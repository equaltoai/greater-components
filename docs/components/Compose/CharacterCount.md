...
<Compose.Root handlers={{ onSubmit: handleSubmit }} config={{ characterLimit: 500 }}>
...
<div class="multi-composer">
	<div class="composer-card">
		...
		<Compose.Root handlers={{ onSubmit: handleTweet }} config={{ characterLimit: 280 }}>
	...
	<div class="composer-card">
		...
		<Compose.Root handlers={{ onSubmit: handleThread }} config={{ characterLimit: 500 }}>
	...
	<div class="composer-card">
		...
		<Compose.Root handlers={{ onSubmit: handleNote }} config={{ characterLimit: 5000 }}>
...
<div class="detailed-composer">
	<Compose.Root handlers={{ onSubmit: handleSubmit }} config={{ characterLimit: 500 }}>
		<Compose.Editor autofocus oninput={(e) => handleContentChange(e.target.value)} />
...
<div class="threshold-composer">
	...
	<Compose.Root handlers={{ onSubmit: handleSubmit }} config={{ characterLimit: 280 }}>
		<Compose.Editor autofocus oninput={(e) => updatePercentage(e.target.value, 280)} />
...
<script lang="ts">
	...
	const examples = [
		...
		{
			text: 'Check https://example.com/very/long/path',
			expected: 29,
			note: 'With URL (weighted as ~23 chars)',
		},
	];
...
describe('Unicode Character Counting', () => {
	...
	it('weights URLs correctly', () => {
		const result = countWeightedCharacters('Check https://example.com/very/long/url/path', {
			useUrlWeighting: true,
			urlWeight: 23,
		});
		// "Check " (6) + URL (23) = 29

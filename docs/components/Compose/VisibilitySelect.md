...
<script lang="ts">
	...
	function handleContentChange(content: string) {
		...
		showWarning = context.state.visibility === 'public' && contentSensitivity !== 'safe';
	}
...
<div class="compose-visibility-select">
	...
	<select id="compose-visibility" aria-label="Post visibility">
...
function getActivityPubRecipients(post, user) {
	switch (post.visibility) {
		case 'public':
			return ['https://www.w3.org/ns/activitystreams#Public', ...user.followers];
		case 'unlisted':

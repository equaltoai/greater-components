...
<div class="notifications-container">
	...
	<Notifications.Root notifications={filteredNotifications} {handlers} {config}>
		<Notifications.Filter />
...
<script lang="ts">
	...
	const unreadCounts = $derived({
		...
		follows: notifications.filter(
			(n) => (n.type === 'follow' || n.type === 'follow_request') && !n.read
		).length,
...
<style>
	...
	.filter-badge--mentions {
		background: #8b5cf6;
	}
	.filter-badge--follows {
		background: #3b82f6;
	}
	.filter-badge--boosts {
		background: #10b981;
...
<div class="persistent-notifications">
	<header class="header">
		...
		<div class="header-info">
			...
			{#if activeFilter !== 'all'}
				<button class="reset-filter" onclick={() => handlers.onFilterChange('all')}>
					Show All
				</button>
			{/if}
		</div>
	...
	<Notifications.Root {notifications} {handlers} config={{ filter: activeFilter }}>
		<Notifications.Filter />

		<div class="notifications-scroll" bind:this={scrollContainer} onscroll={handleScroll}>
			{#if loading}
				<div class="loading">
...
<script lang="ts">
	...
	const activeFilter = $derived(
		($page.url.searchParams.get('filter') as
			| 'all'
			| 'mentions'
			| 'follows'
			| 'boosts'
...
<div class="url-filtered-notifications">
	<Notifications.Root {notifications} {handlers} config={{ filter: activeFilter }}>
		...
		<div class="filter-info">
			...
			<p>
				Showing {filteredNotifications.length} notification{filteredNotifications.length === 1
					? ''
					: 's'}
			</p>
		</div>
	...
	<Notifications.Root {notifications} {handlers} config={{ filter: activeFilter }}>
		...
		<div class="statistics-panel">
			...
			<div class="progress-bar">
				<div class="progress-fill" style="width: {getReadPercentage(activeFilter)}%"></div>
			</div>

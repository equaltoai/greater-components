...
<div class="grouped-notifications-demo">
	...
	<Notifications.Root {notifications} {groups} {handlers} {config}>
...
<Notifications.Root notifications={group.notifications} groups={[group]} {handlers}>
	<Notifications.Group {group}>
		{#snippet children()}
			<article class="custom-group">
				...
				<div class="custom-group__avatars">
					{#each group.accounts.slice(0, 5) as account, i}
						<div class="custom-group__avatar" style="z-index: {5 - i}" title={account.displayName}>
				...
				<button class="custom-group__expand" onclick={() => (expanded = !expanded)}>
...
<script lang="ts">
	...
	async function acceptRequest(accountId: string, event: Event) {
		...
		try {
			...
			followRequestGroup.accounts = followRequestGroup.accounts.filter((a) => a.id !== accountId);
		...
		<div class="interactive-group">
			<div class="interactive-group__header">
				<div class="interactive-group__info">
					<h3>
						{followRequestGroup.count} New Follow {followRequestGroup.count === 1
							? 'Request'
							: 'Requests'}
					</h3>
				...
				<div class="time-formatted-group__timeline-viz">
					{#each group.notifications as notification, i}
						<div
							...
							title="{notification.account.displayName} - {formatDetailedTime(
...
```html
<article role="article" aria-label="{names} {action}">
	<!-- content -->
</article>
```

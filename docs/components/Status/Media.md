...
<style>
	...
	:global(
		.status-media:has(.status-media__item:nth-child(3):last-child) .status-media__item:nth-child(3)
	) {
		grid-column: span 2;
	}
		...
		{#snippet media()}
			<div class="audio-player">
				<div class="audio-cover">
					<img src={statusWithAudio.mediaAttachments[0].previewUrl} alt="Podcast cover" />
				</div>
			...
			<div class="sensitive-media-container">
				{#if !showSensitiveMedia}
					<div class="sensitive-overlay">
						<div class="sensitive-warning">
							...
							<button class="show-media-btn" onclick={() => (showSensitiveMedia = true)}>
								Show Media
							</button>
						</div>
					</div>
				...
				<div class:hidden={!showSensitiveMedia} class:blurred={!showSensitiveMedia}>
					{#each sensitiveStatus.mediaAttachments as attachment}
						<img src={attachment.previewUrl} alt={attachment.description || ''} loading="lazy" />
					{/each}
				</div>
			</div>
		{/snippet}
...
```html
<img src="photo.jpg" alt="Sunset over ocean with orange and pink clouds" loading="lazy" />
```

		...
		<div class="admin-layout">
			...
			<aside class="admin-sidebar">
				...
				<nav class="admin-sidebar__nav">
					...
					<button class:active={activeTab === 'users'} onclick={() => (activeTab = 'users')}>
						👥 Users
					</button>
					<button class:active={activeTab === 'reports'} onclick={() => (activeTab = 'reports')}>
						🚩 Reports
					</button>
					<button
...
import type {
	...
	AdminStats,
	AdminUser,
} from '@equaltoai/greater-components-fediverse/Admin';
...
<script lang="ts">
	...
	async function withRetry<T>(fn: () => Promise<T>, retries = maxRetries): Promise<T> {
		try {
			return await fn();
		} catch (err) {
			if (retries > 0) {
...
const rolePermissions = {
	...
	moderator: [Permission.VIEW_STATS, Permission.MANAGE_REPORTS],
};

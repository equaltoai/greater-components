...
<div class="2fa-setup-flow">
	{#if step === 'setup'}
		...
		>
			<Auth.TwoFactorSetup email="user@example.com" onComplete={() => (step = 'codes')} />
		</Auth.Root>
	{:else if step === 'codes'}
		<Auth.Root>
			<Auth.BackupCodes codes={backupCodes} onConfirmed={handleConfirmed} isSetup={true} />
		</Auth.Root>
	{:else}
...
<div class="security-settings">
	...
	<section class="settings-section">
		...
		{:else}
			...
			>
				<Auth.BackupCodes codes={backupCodes} isSetup={false} />
			</Auth.Root>
		{/if}
	</section>
</div>
...
<Auth.Root>
	<div class="codes-status">
		...
		{#if codesRemaining <= 3}
			<div class="status-warning">⚠️ Running low on backup codes. Consider regenerating them.</div>
		{/if}
	</div>

	<Auth.BackupCodes codes={backupCodes.map((c) => c.code)} onRegenerate={handleRegenerate} />
		...
		{#if showEncryptedDownload}
			<div class="encryption-form">
				...
				<div class="encryption-actions">
					<button onclick={handleDownloadEncrypted}> Download Encrypted </button>
					<button onclick={() => (showEncryptedDownload = false)}> Cancel </button>
				</div>
			</div>
		{/if}
	</div>
		...
		{#if showQRCode}
			<div class="qr-code-display">
				...
				<div class="qr-actions">
					...
					<button onclick={() => (showQRCode = false)}> Close </button>
				</div>
			</div>
		{/if}
	</div>

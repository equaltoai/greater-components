	...
	const handlers = {
		onFollow: async (userId: string) => {
			...
			try {
				...
				followers = followers.map((f) => (f.id === userId ? { ...f, following: true } : f));
			} catch (error) {
		...
		onUnfollow: async (userId: string) => {
			...
			try {
				...
				// Update following status
				followers = followers.map((f) => (f.id === userId ? { ...f, following: false } : f));
			} catch (error) {
...
export async function POST(
	...
	try {
		...
		return new Response(JSON.stringify({ message: 'Follower removed' }), {
			headers: { 'Content-Type': 'application/json' },
		});
	} catch (error) {
		console.error('Failed to remove follower:', error);
...
<div class="infinite-scroll-followers">
	...
	{#if hasMore}
		<div bind:this={loadMoreTrigger} class="load-more-trigger">
			...
			{:else}
				<button onclick={loadFollowers} class="load-more-button"> Load More </button>
			{/if}
		</div>
	{/if}
</div>

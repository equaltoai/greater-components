...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!url.startsWith('http://') && !url.startsWith('https://')) {
    return new Response(
      JSON.stringify({ verified: false, error: 'Invalid URL' }),
  ...
  try {
    ...
    if (verified) {
      ...
      return new Response(
        JSON.stringify({ verified: true, verifiedAt }),
        { headers: { 'Content-Type': 'application/json' } }
      );
    } else {
      return new Response(
...
<script lang="ts">
  ...
  async function verifyAllFields() {
    ...
    const urlFields = fields.filter(f => 
      f.value.startsWith('http') && !f.verifiedAt
    );
    ...
    const verifications = urlFields.map(async (field) => {
      try {
        ...
        if (result.verified) {
          ...
          fields = fields.map(f =>
            f.name === field.name
              ? { ...f, verifiedAt: result.verifiedAt }
              : f
          );
        } else {
          results.set(field.name, {

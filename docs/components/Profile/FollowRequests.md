...
<Profile.Root profile={profileData} {handlers} isOwnProfile={true}>
	<Profile.FollowRequests {requests} showBatchActions={true} enableFiltering={true} />
</Profile.Root>
```
		...
		{#each requests as request}
			...
			<div class="request-card" class:expanded={isExpanded}>
				<div class="request-header">
					...
					>
						<svg viewBox="0 0 24 24" fill="currentColor" class:rotated={isExpanded}>
				...
				<div class="request-actions">
					<button class="reject-button" onclick={() => handleReject(request.id)}> Reject </button>
					<button class="approve-button" onclick={() => handleApprove(request.id)}>
						Approve
					</button>
				</div>
			</div>
...
<script lang="ts">
	...
	const allSelected = $derived(requests.length > 0 && selectedIds.size === requests.length);
		...
		<div class="batch-controls">
			...
			{#if hasSelection}
				<div class="batch-actions">
					...
					<button class="batch-reject" onclick={handleRejectSelected} disabled={processing}>
						{#if processing}
							<span class="spinner"></span>
						{/if}
						Reject Selected
					</button>
					<button class="batch-approve" onclick={handleApproveSelected} disabled={processing}>
						{#if processing}
							<span class="spinner"></span>
						{/if}
						Approve Selected
		...
		<div class="requests-list">
			{#each requests as request}
				<div class="request-item" class:selected={selectedIds.has(request.id)}>
					...
					<img src={request.account.avatar} alt={request.account.displayName} class="avatar" />
...
export async function POST(
	...
	try {
		...
		if (existingFollow) {
			...
			return new Response(JSON.stringify({ message: 'Already following' }), {
		...
		return new Response(JSON.stringify({ message: 'Follow request approved' }), {
			headers: { 'Content-Type': 'application/json' },
		});
	} catch (error) {
...
export async function POST(
	...
	try {
		...
		return new Response(JSON.stringify({ message: 'Follow request rejected' }), {
			headers: { 'Content-Type': 'application/json' },
		});
	} catch (error) {

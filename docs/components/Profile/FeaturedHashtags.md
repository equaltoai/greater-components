...
<div class="hashtags-page">
	<header class="page-header">
		...
		<p class="description">Highlight topics you frequently discuss</p>
	</header>
...
<script lang="ts">
	...
	async function searchHashtags() {
		...
		try {
			const response = await fetch(`/api/search/hashtags?q=${encodeURIComponent(query)}`, {
				credentials: 'include',
			});
...
<div class="add-hashtag-flow">
	...
	{#if canAddMore}
		<button class="add-hashtag-button" onclick={() => (showAddDialog = true)}>
			<svg viewBox="0 0 24 24" fill="currentColor">
					...
					{:else if searchResults.length > 0}
						<div class="search-results">
							...
							{#each searchResults as result}
								<button class="result-item" onclick={() => addHashtag(result.name)}>
...
export async function DELETE(
	...
	try {
		...
		return new Response(JSON.stringify({ message: 'Featured hashtag removed' }), {
			headers: { 'Content-Type': 'application/json' },
		});
	} catch (error) {
...
<div class="public-hashtags">
	...
	<div class="hashtags-grid">
		{#each hashtags as hashtag}
			<button class="hashtag-card" onclick={() => navigateToHashtag(hashtag.name)}>
...
describe('FeaturedHashtags', () => {
	it('displays featured hashtags', () => {
		const hashtags = [{ name: 'javascript', usageCount: 42 }];

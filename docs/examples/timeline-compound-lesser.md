			...
			posts = posts.map((p) =>
				p.id === status.id
					? {
							...
							favouritesCount: p.favouritesCount + (p.favourited ? -1 : 1),
						}
					: p
			);
		} catch (err) {
...
<div class="home-timeline">
	...
	>
		...
		{:else if posts.length === 0}
			...
			>
				{#snippet action()}
					<button onclick={() => goto('/explore')}> Discover accounts </button>
				{/snippet}
			</Timeline.EmptyState>
		{:else}
...
<Timeline.Root {items} handlers={{ onScroll: handleScroll }}>
	<!-- Timeline items -->
</Timeline.Root>
```

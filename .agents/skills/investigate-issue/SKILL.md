...
Investigation comes before implementation. greater has distinctive investigation dimensions: a shadcn-style CLI distribution that lands source in consumers, pinned contract snapshots that must align with adapter code, a feature → staging → main release flow with manual tag-driven releases, WCAG 2.1 AA-enforcing CI gates, a theming contract via CSS custom properties, and a dual-consumer reality (Lesser-aware features + Mastodon-baseline compatibility).

## Start with memory

Call `memory_recent` first. Scan for prior investigations — component-API evolution lessons, contract-sync edge cases, accessibility-test flakiness patterns, CLI registry subtleties, release-flow lessons (staging→main promotion, manual release workflow). greater is active; prior context accumulates fast.
...
- **Recent deploys / releases / promotions** — feature → staging → main transitions

## Ground the investigation

Your first structural questions are always:

1. **Is this a component API stability issue?** A breaking change landed without major-version coordination, or consumer build broke after `greater update`. Route through `evolve-component-surface` for the API walk.
...
5. **Is this a release-flow issue?** Promotion stuck, manual release workflow failed, tag ancestry is wrong, registry artifact drifted. Route through `release-components`.
...
- `git log` since last known-good state on the affected branch (`staging` / `main`)
...
- For release flow: staging/main PR state; current tags; signed-tag integrity; manual release workflow state
...
- **Release flow (feature → staging → main, manual tag release, CLI registry, signed tags)** → `release-components`
...
Append only when the investigation surfaces something worth remembering — a component-API evolution edge case, a contract-sync mismatch pattern, an accessibility-test flakiness story, a CLI-registry drift observation, a release-flow subtlety (main promotion timing, manual release workflow edge case), a Mastodon-compat boundary, a framework-awkwardness worth upstreaming. Routine "typo" findings aren't memory material. Five meaningful entries beat fifty log-shaped ones.
